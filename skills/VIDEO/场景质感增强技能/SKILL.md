---
name: 场景质感增强技能
description: 场景材质纹理与光影层次增强。覆盖建筑/自然/室内/光影/天气/地面六维增强+景别适配。触发：(1) 导演要求提升场景质感 (2) 写提示词时场景描述不够 (3) 需要环境氛围参考。
---

# 场景质感增强技能

> **创建日期**：2026-06-26
> **更新**：2026-07-06 — 删除所有做旧/做暗内容，整体明亮通透
...
1. **不准写暗** — 全文禁用：暗、黑、幽暗、昏暗、阴影压死、极黑
2. **不准做旧** — 不用风化、锈蚀、氧化、磨损、龟裂、包浆
3. **不准写废墟/古堡/破败类场景**
4. **整体明亮通透** — PBR材质反光补足暗面细节，不靠压暗制造氛围
5. ✅ 可写：体积光、AO、大气透视、材质纹理自然变化、光影层次
...
| 问题 | 根因 | 解决方向 |
|------|------|---------|
| 场景像游戏贴图 | 材质表面太光滑，无纹理细节 | 加材质纹理/凹凸/ambient occlusion |
...
| 室内像样板间 | 完美无使用痕迹 | 加自然落灰/使用痕迹（不过度做旧） |
...
#### 中式建筑
```
汉白玉石基表面微颗粒纹理，石材自然纹理可见，石阶边缘圆润，
红色漆柱表面漆面哑光非镜面反光，柱身漆面微微泛光，
琉璃瓦逐片独立反光而非均匀一片，瓦片间缝隙阴影可见，
斗拱木构榫卯接缝清晰，木材表面纵向纹理和节疤可见，
檐下阴影区 ambient occlusion 加深形成纵深层次。
```
- 追加关键词：`stone grain texture`, `wood grain detail`, `roof tile individual specular highlights`, `mortise and tenon joint detail`
...
金属雕像表面哑光均匀反光，接缝铸造线隐约可见，
大理石雕像次表面散射微透光，表面蜡质柔和反光，
基座石材与雕像材质纹理对比分明。
```
- 追加关键词：`stone carving chisel mark texture`, `metal statue brushed reflection`, `marble subsurface scattering`

#### 建筑材料通用增强词库

| 材质 | 中文增强 | 英文关键词 |
|------|---------|-----------|
| 石材 | 表面微颗粒纹理，接缝阴影 | `stone grain texture`, `mortar joint shadow` |
| 木材 | 纵向纹理，节疤，哑光漆面 | `wood grain direction`, `knot detail`, `matte varnish` |
| 金属 | 拉丝纹理，铆钉反光 | `brushed metal grain`, `rivet highlight` |
| 玻璃 | 非均匀反射，微扭曲，边缘折射 | `non-uniform glass reflection`, `edge refraction` |
...
枝干分叉处节疤纹理。
```
- 追加关键词：`bark texture`, `leaves light direction color variation`, `Tyndall effect forest beams`, `branching natural asymmetry`
...
花茎绒毛/刺节纹理可见。
```
- 追加关键词：`petal surface micro-texture`, `stamen pollen granules visible`, `natural petal edge irregularity`
...
```
- 追加关键词：`water surface micro-ripple non-uniform reflection`, `suspended particles in water`, `white water foam detail`, `underwater refraction distortion`
...
地毯纤维粗细可见，踩踏区域压痕。
```
- 追加关键词：`wall micro-texture brush marks`, `baseboard gap shadow`, `floorboard individual grain variation`, `floor use pattern`
...
```
- 追加关键词：`furniture wood open grain`, `fabric weave on upholstery`, `leather pore texture`, `natural use shape`

#### 道具/摆件
```
瓷器釉面微气泡，非完美工厂新品，
书籍封皮微磨损边角，书页自然泛黄渐变，
金属器皿表面使用划痕微反光，底部接触面自然痕迹，
植物叶片自然朝向光源非均匀生长，
玻璃器皿边缘绿色折射，表面轻微指纹或灰尘。
```
- 追加关键词：`ceramic glaze detail`, `book edge natural use`, `prop surface detail`, `glass rim green tint refraction`
...
|------|------------|---------|---------------|
| 大特写 ECU | 材料表面纹理、水珠 | 微观材质纹理 | 整体建筑形态 |
...
| 沙漠/戈壁 | 地面·沙地 + 天气·风 + 自然·天空 | 光影·haze |
...
```
【场景】
中式庭院，汉白玉石基表面微颗粒纹理，红色漆柱哑光漆面，
琉璃瓦逐片独立反光，檐下阴影区 ambient occlusion 加深层次，
阳光穿过树叶间隙形成可见体积光束，
...
| 问题 | 错误写法 | 正确写法 |
|------|---------|---------|
| 场景太空洞 | "中式庭院" | "中式庭院，汉白玉石基表面微颗粒纹理，红色漆柱哑光漆面..." |
...
| 室内样板间 | "干净明亮的房间" | "房间墙面微凹凸纹理，木地板自然光泽变化，窗帘自然褶皱重力垂坠" |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0 | 2026-06-26 | 初始版本，六大模块 + 景别适配 + 场景快速适配 + 避坑指南 |
| v2.0 | 2026-07-06 | 删除所有做旧做暗内容：废墟/古堡模块删除、风化锈蚀氧化磨损龟裂包浆全删、暗部黑字全删、整体改为明亮通透 |
