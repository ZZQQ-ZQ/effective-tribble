---
name: 视觉光效系统
description: 电影光效写法全栈。Part A 31种打光系统→Part B 景别匹配+质感公式→Part C 色温控制+美学约束→Part D 光影三层写作法→Part E Seedance实例→Part F 标准化公式→Part G 检查清单→Part H 核心原则。触发：(1) 写分镜光效描述 (2) 定光源色温 (3) 出稿前光效检查 (4) 需要光效写作参考。
---

# 视觉光效系统 — 电影光效写法全栈

> **创建日期**：2026-07-05 | **重组**：2026-07-30（Part A风格选择→已迁移至影视风格技能·附录A）
> **来源**：全网调研 + AbleGPT 31种打光 + MapleShaw深度美学 + 传奇项目实战

---

# Part A：31种打光系统

## A1. 方向光（8种）

| # | 名称 | 光位 | 效果 | 提示词片段 |
|---|------|------|------|-----------|
| 1 | 正面光 | 镜头同方向正前 | 均匀照亮，无阴影 | `flat frontal lighting, evenly lit face` |
| 2 | 顶光 | 正上方垂直下 | 眼窝/鼻下阴影，压迫感 | `top-down harsh light casting deep eye socket shadows` |
| 3 | 底光 | 下方往上打 | 反派光，诡异/恐怖 | `under-lighting from below, unsettling shadows` |
| 4 | 侧光 | 侧面90°打 | 半明半暗，戏剧冲突 | `side lighting splitting face into lit and shadow halves` |
| 5 | 轮廓光/边缘光 | 后方斜侧 | 人物边缘高亮，与背景分离 | `rim light outlining figure's edge, separation from dark background` |
| 6 | 伦勃朗光 | 侧上45° | 一侧脸颊三角光斑 | `Rembrandt lighting, triangular light patch under shadow-side eye` |
| 7 | 蝴蝶光 | 前上45° | 鼻下蝴蝶形阴影，时尚 | `butterfly lighting, butterfly-shaped shadow beneath nose` |
| 7b | 环形光 | 侧上30-45° | 鼻影在脸颊形成小环形，面部均匀自然 | `loop lighting, small loop-shaped nose shadow on cheek, even natural illumination` |
| 8 | 分割光 | 侧面90° | 面部精确一半亮一半暗 | `split lighting, half face illuminated half in deep shadow` |

## A2. 氛围光（7种）

| # | 名称 | 色温 | 氛围 | 提示词片段 |
|---|------|------|------|-----------|
| 1 | 金色时刻 | 3200-4000K | 日落前暖金低角度，长影 | `golden hour, warm amber sunlight, long dramatic shadows` |
| 2 | 蓝色时刻 | 8000-10000K | 日落后冷蓝，静谧 | `blue hour, cool twilight tones, deep indigo sky gradient` |
| 3 | 阴天漫射 | 5500-6500K | 柔和均匀，无硬阴影 | `overcast diffused light, soft shadowless illumination` |
| 4 | 斑驳树影 | 4500-5500K | 阳光透过树叶，光斑跳动 | `dappled sunlight filtering through tree canopy, shifting light patches` |
| 5 | 月光 | 4000-5000K | 冷白银色，夜寂静谧 | `moonlight, cool silvery-blue illumination, pale highlights` |
| 6 | 霓虹双色 | 混合 | 赛博朋克 | `dual-tone neon, warm magenta key + cool cyan rim` |
| 7 | 烛光 | 1800-2200K | 暖橙跳动 | `candlelight, warm flickering amber glow, intimate illumination` |

## A3. 电影光（7种）

| # | 名称 | 特点 | 提示词片段 |
|---|------|------|-----------|
| 1 | 三点布光 | key+fill+rim，标准电影光 | `three-point lighting, key defining form, fill controlling shadows, rim for separation` |
| 2 | 高调光 | 明亮干净，商业广告 | `high-key lighting, bright clean illumination, minimal shadows` |
| 3 | 低调光 | 暗部主导，悬疑氛围 | `low-key lighting, deep shadows dominating frame, dramatic contrast` |
| 4 | 明暗对照 | 强对比光影，经典油画感 | `chiaroscuro lighting, dramatic light-shadow contrast, sculptural illumination` |
| 5 | 有源光 | 光有明确来源（窗/灯/火） | `motivated lighting from visible window source, naturalistic illumination` |
| 6 | 实景光源 | 画面内可见光源 | `practical light visible in frame, diegetic light source illuminating scene` |
| 7 | 背景渐变光 | 背景从中心亮区自然过渡到边缘暗区，人物突出 | `vignette background lighting, graduated illumination bright at center fading to edges, subject pop` |

## A4. 特殊效果光（15种）

| # | 名称 | 效果 | 提示词片段 |
|---|------|------|-----------|
| 1 | 硬光 | 边缘锐利，强对比 | `hard directional light, crisp defined shadows with sharp edges` |
| 2 | 柔光 | 大光源漫射，过渡柔和 | `soft diffused lighting, gentle shadow falloff, smooth transitions` |
| 3 | 体积光/上帝光 | 空气中可见光柱 | `volumetric lighting, visible light beams cutting through air, god rays` |
| 4 | 轮廓背光 | 人物边缘发亮 | `strong rim backlight outlining subject, hair catching edge light` |
| 5 | 发际光 | 发丝背光勾勒 | `hair light creating luminous halo around head from behind` |
| 6 | 百叶窗光 | 规整明暗条纹，都市悬疑电影感 | `venetian blind shadows, striped light pattern across face and wall, film noir interrogation feel` |
| 7 | 手机屏幕光 | 小面积冷蓝近光，仅照亮局部人脸，背景压暗 | `phone screen glow, small cool-blue light source illuminating only face, dark surroundings` |
| 8 | 街灯光 | 单点暖光从上打下，强烈明暗反差 | `streetlight, single warm overhead pool of light, stark contrast against dark street` |
| 9 | 晨雾逆光 | 柔和穿透雾气的光线包裹人物边缘，清晨诗意通透 | `morning mist backlight, soft diffused light wrapping subject edges through fog, ethereal dawn glow` |
| 10 | 雨夜反射光 | 潮湿地面的环境反光，冷调都市孤独感 | `rainy night reflections, wet pavement mirroring city lights, cool blue ambient bounce from below` |
| 11 | 舞台追光 | 远距离集中高亮打亮主体，戏剧感强 | `stage spotlight, concentrated beam isolating subject, dramatic falloff to darkness` |
| 12 | 投影光 | 窗格/树影等图案投射人物面部，光影参与构图 | `projected shadow patterns on face, dappled leaf shadows or window grille cast across subject, light as visual element` |
| 13 | 水波反射光 | 流动的晃动光纹扫过人物，夏日梦幻怀旧 | `water caustics, rippling reflected light patterns dancing across subject, dreamy summer nostalgia` |
| 14 | 荧光灯光 | 冷调偏硬顶侧照明，现实空间压抑感 | `fluorescent tube light, cold clinical overhead illumination, slight green cast, harsh unflattering shadows` |
| 15 | 门缝光 | 极窄定向光束照亮局部，周围大面积压黑 | `sliver of light through door crack, thin razor-sharp beam cutting darkness, high tension suspense` |

## A5. 环境特效光（5种）

| # | 名称 | 效果 | 提示词片段 |
|---|------|------|-----------|
| 1 | 雾霭 | 空气透视，远处朦胧 | `atmospheric haze softening distant details, layered depth through fog` |
| 2 | 浮尘粒子 | 光束中可见微尘 | `dust motes suspended in light beams, particles drifting slowly` |
| 3 | 漏光 | 胶片意外曝光效果 | `film light leak, warm amber streaks bleeding into frame edges` |
| 4 | 镜头光晕 | 光源直射镜头 | `lens flare, multi-element anamorphic flare streaks across frame` |
| 5 | 高光溢出 | 亮部柔光扩散 | `bloom effect, highlights softly bleeding into surrounding areas` |

---

# Part B：景别×光效 + 质感公式

## B1. 景别 × 光效匹配

| 景别 | 光效重点 | 写入提示词的关键要素 |
|------|---------|-------------------|
| 极特写 ECU | 虹膜高光/皮肤毛孔/光影过渡 | `iris catchlight, pore-level texture, single soft source` |
| 特写 CU | 面部单侧柔光/伦勃朗三角/发丝背光 | `one-sided soft key, Rembrandt triangle, hair backlight halo` |
| 近景 MCU | 三点布光完整可见 | `three-point visible, key direction clear, fill controlling density` |
| 中景 MS | 人物+环境光影统一/体积光显形 | `subject and environment lit as one, volumetric beams visible` |
| 全景 LS | 丁达尔光柱/大气透视/阴影层次 | `god rays through openings, atmospheric depth, layered shadows` |
| 大全景 ELS | 全局光影基调/天空梯度/远景衰减 | `global light direction, sky gradient, distant color falloff` |

## B2. 电影感质感公式

```
主体 + 场景 + 动作
+ 材质质感（皮肤毛孔/面料肌理/金属反光）
+ 光线质感（方向/色温/软硬/体积光/阴影层次）
+ 色彩质感（橙青/褪色胶片/冷暖对比/饱和度层次）
+ 空气质感（雾/尘/大气透视/景深衰减）
+ 镜头运动（固定/跟拍/环绕/推拉/手持）
+ 情绪氛围（宁静/压迫/温暖/冷寂/紧张）
```

### 快速套用模板
```
电影级画质，[XXmm]镜头，[XX光]从[方向]打向[主体]，
在[背景]中形成[光影效果]，空气中有[粒子/雾/尘]缓慢飘动，
[材质]表面反射[光色]，[色彩风格]，浅景深背景虚化，
镜头[运镜方式]，[情绪氛围]
```

## B3. 分维度光效增强速查

| 维度 | 关键描述词 |
|------|-----------|
| 🧑 皮肤 | `保留毛孔纹理` `不磨皮` `面部单侧柔光` `伦勃朗三角光斑` |
| 👁 眼睛 | `虹膜高光点闪烁` `眼神灵动` `缓慢自然眨眼` `catchlight反射` |
| 💇 头发 | `根根分明发丝` `粗细错落` `环境反光` `发丝背光高亮` `不结块` |
| ⚔️ 金属武器 | `刃面映出环境碎片` `刃缘高光碎斑` `刀身宽厚重量感` |
| 🛡 金属甲胄 | `表面微划痕反射环境光` `磨砂哑光/镜面高光区域对比` |
| 👘 丝绸 | `高光通透` `光泽流动` `随动作反光位置变化` |
| 👘 皮革 | `哑光微磨` `边缘磨损露出底色` |
| 👘 薄纱 | `半透质感` `层叠光影` `透出下层衣料色彩` |
| 🌳 植物 | `草叶随机朝向` `叶片正反面双色反光` `露珠微棱镜` |
| 🏔 远景 | `大气透视远景色偏蓝` `饱和度随距离衰减` |
| 🌫 空气 | `暖金色光雾` `丁达尔光束斜射` `浮尘粒子缓慢飘动` |
| 📷 长焦(85-135mm) | `背景强力虚化` `空间压缩` `主体与背景分离` |
| 📷 广角(14-24mm) | `人物放大前景开阔` `远景点透视压缩` |

---

# Part C：色彩温度控制 + 美学约束

## C1. 色彩温度控制

| 光源 | 色温 (K) | 色调 | 场景 |
|------|---------|------|------|
| 烛光 | 1800-2200 | 暖橙 | 古堡/山洞/夜晚室内 |
| 日出日落 | 2500-3500 | 金橙 | 金色时刻/浪漫/怀旧 |
| 正午阳光 | 5000-5500 | 白 | 晴天户外 |
| 阴天 | 6000-6500 | 微冷 | 柔和漫射 |
| 蓝天阴影 | 7000-8000 | 冷蓝 | 阴影区 |
| 月光 | 4000-5000 | 冷银 | 夜景 |
| 霓虹灯 | 混合 | 多色 | 赛博朋克/都市夜 |

**色彩对比公式**：
- `暖色主体 + 冷色背景` → 人物突出
- `橙肤色 + 青阴影` → 好莱坞橙青调
- `高饱和高光 + 低饱和阴影` → 立体层次

## C2. 渲染品质三档（MapleShaw·对标Octane/V-Ray）

| 档位 | 字数 | 关键词 |
|------|------|--------|
| 基础 | ~20字 | `Octane渲染品质，全局光照，材质细节真实，物理正确光影` |
| 进阶 | ~50字 | `+ SSS skin detail, micro-surface imperfections, anisotropic highlights, volumetric atmosphere, cinematic lens flare` |
| 极致 | ~80字 | `+ 物理正确光线衰减, 真实焦散, ACEScg色彩空间, 运动模糊物理准确, 景深散景真实光学模拟` |

## C3. 六组冷暖色调

| 色调 | 冷色(暗部) | 暖色(亮部) | 情绪 | 场景 |
|------|-----------|-----------|------|------|
| 赛博霓虹 | 深空蓝+电光紫 | 霓虹红+能量橙 | 紧张→爆发 | 科技/夜间 |
| 极简白金 | 苹果白+铂金灰 | 金色+炭黑 | 宁静→聚焦 | 奢侈品 |
| 冰火对冲 | 深海蓝+钴蓝 | 烈焰红+橙黄 | 对峙→冲突 | 对决/竞技 |
| 暮光渐变 | 暮紫+深青 | 落霞粉+暮金 | 舒缓→回味 | 情感/MV |
| 极简黑白 | 纯黑 | 纯白+点缀色 | 克制→冲击 | 先锋艺术 |

## C4. 材质微观（6种）

| 材质 | 关键词 |
|------|--------|
| 金属 | 各向异性高光+微划痕+真实反射 |
| 皮肤 | 毛孔+次表面散射SSS+微血管 |
| 玻璃 | 色散彩虹+焦散+折射率IOR 1.52 |
| 织物 | 纤维级编织+绒毛+光散射 |
| 液体 | 弯月面+粘度+飞溅物理 |
| 塑料/树脂 | 多层涂层+底层颗粒+清漆反射 |

---

# Part D：光影三层结构写作法（MapleShaw）

> **写光效不笼统，拆成三层——每层一句话，覆盖全维度。**

## 模板

```
光影：{光源层}，{光行为层}，{色调层}
```

## 第一层：光源（是什么光、从哪里来）

| 场景类型 | 光源词 |
|---------|--------|
| 灾难/动作 | 暴雨逆光、爆炸橙红火光、闪电侧光 |
| 奇幻/科幻 | 怪兽熔岩自发光、能量球蓝白光、飞船引擎尾焰光 |
| 都市/夜景 | 霓虹灯漫射、建筑玻璃幕墙反射、车灯流光 |
| 自然/写实 | 暴风雨漫射天光、黄金时刻侧逆光、月光冷辉、火焰跳动暖光 |
| 仙侠/古风 | 灵光逸散、符阵金芒、仙气氤氲、烛火摇曳 |

## 第二层：光的行为（光如何与材质/大气互动）

```
薄雾柔化高光 / 雾层强化阴影层次 / 烟尘散射光线 /
玻璃折射彩虹光斑 / 金属反射爆炸火光 / 雨水折射霓虹 /
丁达尔效应清晰 / 体积光穿透尘埃
```

## 第三层：色调（整体冷暖和色彩关系）

| 风格 | 色调公式 |
|------|---------|
| 灾难/压迫 | 冷蓝底调 + 熔岩红高光 |
| 赛博朋克 | 冷蓝底调 + 霓虹紫红高光 |
| 奇幻/仙侠 | 青灰底调 + 金色荧光高光 |
| 末日/恐怖 | 灰绿底调 + 暗红点缀 |
| 暖色/史诗 | 暗棕底调 + 橙金高光 |
| 现代/写实 | 中性底调 + 暖白高光 |
| CG游戏 | 锐利干净底调 + 金属亮银高光 |

## 使用示例

```
光影：穹顶破洞暖金日光斜穿（光源层），
薄雾柔化高光但强化阴影层次、丁达尔效应清晰、金属铠甲反射暖金（光行为层），
暖金底调+橙金高光（色调层）
```

### 简化版（日常分镜）

写分镜时间紧张时，三合一压缩：
```
光影：暖金日光逆光穿尘，薄雾柔化阴影层次，暖金底调
```

---

# Part E：Seedance 光效提示词实例

## 户外森林场景
```
电影级画质，35mm变形宽银幕，正午暖金色阳光从左侧高大树冠缝隙穿透，
在草地上投下斜向丁达尔光柱，光雾中浮尘粒子缓慢上升飘动，
战士铠甲表面反射碎金高光，刀身刃面映出环境碎片，
暖金阳光与翠绿草地形成强烈色彩对比，浅景深远景森林虚化在光雾中，
地平线处天空从暖金过渡到淡蓝，宁静中带着空旷孤寂
```

## 战斗场景
```
电影级画质，手持跟拍，侧逆硬光从画右上方斜射战场，
体积光穿透扬起的尘土形成分层光幕，铠甲反光随动作闪烁跳跃，
刀刃碰撞瞬间产生碎芒四溅，火星在高光区拖出短弧线，
暖橙火花与冷蓝刀光双色交织，镜头脏镜弧线划过画面
```

## 室内人物特写
```
电影级画质，85mm长焦镜头，单侧柔光从右侧窗斜射人物面部，
形成伦勃朗三角光斑在左侧颧骨下，眼窝阴影深邃眼神有光，
发丝背光勾勒金色轮廓，皮肤毛孔纹理在亮面保留暗面柔和过渡，
背景虚化为暖色bokeh光斑，浅景深，f/1.8
```

---

# Part F：光影色调标准化公式（工业化批量套用）

> **来源**：2026-07-14 导演提供 · AI漫剧工业化制作参考

## F1. 通用公式

```
[光源类型] + [色温数值] + [光线照射方向] + [视觉聚焦区域] + [情绪渲染效果] + [整体画面氛围]
```

## F2. 6类主流场景色温速查

| 场景类型 | 光源类型 | 色温 | 光线方向 | 聚焦区域 | 情绪渲染 | 整体氛围 |
|---------|---------|------|---------|---------|---------|---------|
| **日常白昼** | 窗边柔和自然光 | 5500K | 侧光或正面散射 | 人物全身/中景 | 自然舒适 | 明亮通透，阴影柔和 |
| **黄金日落** | 夕阳暖调逆光 | 3200K | 逆光+发丝光 | 人物轮廓+发丝边缘 | 温暖浪漫 | 暖金色笼罩，长影拉伸 |
| **蓝调时刻** | 天空冷调蓝光+室内暖光互补 | 7000K | 冷暖双方向对冲 | 人物面部+窗景 | 静谧忧郁 | 冷暖交融，蓝紫渐变天空 |
| **夜间室内** | 台灯/蜡烛暖黄侧光 | 2600K | 单侧低角度 | 人物面部+手部动作 | 亲密/沉重/悲伤 | 暖黄光晕包围，暗部渐隐 |
| **悬疑惊悚** | 单源硬质光线 | 4000K | 顶光或单侧硬光 | 人物局部（半明半暗） | 紧张/不安 | 高明暗对比，深阴影切割画面 |
| **赛博朋克** | 红蓝霓虹对比光源 | 混合冷暖 | 多点环境光 | 人物剪影/轮廓+光污染环境 | 酷炫/压迫 | 湿地面反射霓虹，密集人工光 |

## F3. 实战套用示例

### 夜间煤油灯场景
```
光源：夜间煤油灯暖黄色侧光
色温：2600K
方向：从人物右侧斜下45°打向面部和手部
聚焦：人物面部表情+手部细微动作
情绪：柔和光影过渡，烘托出不舍离别的沉重氛围感
整体：暖黄光晕包裹人物，背景渐隐在黑暗中
```

### 黄金日落对峙
```
光源：夕阳暖金色逆光
色温：3200K
方向：从人物背后低角度射入镜头
聚焦：二人轮廓剪影+武器边缘高光
情绪：剑拔弩张中的壮烈悲凉
整体：暖金逆光笼罩战场，人物剪影被发丝光勾勒
```

### 赛博都市夜战
```
光源：红蓝霓虹对比光源+湿地面反射
色温：混合冷暖
方向：建筑霓虹招牌上方+地面反射下方
聚焦：人物高速移动的剪影+刀光轨迹
情绪：压迫感与速度感并存
整体：高对比冷暖撞色，密集霓虹光污染，雨夜湿地面双重反射
```

---

# Part G：光效快速检查清单（出稿前必过）

```
[ ] 光源方向是否明确？（左/右/上/下/前/后斜侧？）
[ ] 光源色温是否合理？（日/夜/室内/烛光对应K值？）
[ ] 光质是硬是柔？（点光源→硬，大面光源→柔）
[ ] 有没有体积光？（树冠/窗/门缝→光柱）
[ ] 有没有空气介质？（雾/尘/烟→可见光路）
[ ] 人物身上有没有对应光影？（受光面+阴影面+轮廓光）
[ ] 道具表面有没有环境反光？（金属→反射，布料→吸收）
[ ] 景别与光效匹配吗？（特写→面部光影，全景→全局光向）
[ ] 阴影层次清晰吗？（近深远浅，不糊成一团黑）
[ ] 有没有镜头光学质感？（光晕/bokeh/色散/柔焦？）
```

---

# Part H：核心原则

1. **外部确定光线** — 写实类场景必须是有源光（窗/灯/火/穹顶），不用模糊形容词
2. **光效写入分镜内容不另起行** — 光效描述自然嵌入画面内动中
3. **景别决定光效细节量** — 特写写面部光影，全景写全局光向
4. **冷暖层次** — 主光源暖色→冷色阴影区用PBR材质反光自然补足
5. **体积光是真实感的低成本替代** — 丁达尔光束/扬尘光照/雾气散射比单纯写"真实感"有效10倍
6. **色温必须对应光源类型** — 日光5600K/烛光1800K/月光4000K/金色时刻3200K，不混搭

---

## 版本

- **v1.0** (2026-07-05)：Part A四维选风格+Part B 31种打光，融合光效提升技能
- **v2.0** (2026-07-30)：重组——Part A风格选择→迁移至影视风格技能·附录A；视觉光效专精光效写法，8个Part重新编号去重
